...
# --- Data structures ---
class StackFrame:
    __slots__ = ('tid','addr','key','start_time','start_e0','start_e1')
    def __init__(self, tid, addr, start_time, start_e0, start_e1):
        self.tid = tid
        self.addr = addr
        self.key = (tid, addr)
...
class FunctionRecord:
    __slots__ = ('name','tid','addr','total_time','times_called','ev0','ev1')
    def __init__(self, name, tid, addr):
        self.name = name
        self.tid = tid
        self.addr = addr
        self.total_time = 0
...
# --- Global mappings ---
functions = {}  # (tid, addr) -> FunctionRecord
child_calls = defaultdict(lambda: defaultdict(ChildCall))  # parent_key -> (child_key -> ChildCall)
call_stacks = defaultdict(list)  # tid -> list of StackFrame

def node_id_for(key):
    tid, addr = key
    return f"t{tid}_{addr:08x}"
...
class DotManager:
    ...
    def write_node_shapes(self, fp):
        for key, fn in functions.items():
            ...
            child_cycles = sum(cc.total_cycles
                               for callee, cc in child_calls.get(key, {}).items()
                               if callee != key)
            ...
            self.pq.insert(key, pct_inclusive, cum)

            # if recursive, record full cumulative into that child edge
            if key in child_calls.get(key, {}):
                child_calls[key][key].total_cycles = cum

            col   = self.color_from_percent(pct_cum)
            shape = "rectangle" if child_cycles else "ellipse"
            label = f"{fn.name} [T{fn.tid}]\\n{pct_cum:.2f}%\\n"
            children = child_calls.get(key, {})
            ...
            if child_cycles:
                child_ev0_total = sum(cc.ev0 for callee, cc in children.items() if callee != key)
            ...
            if child_cycles:
                child_ev1_total = sum(cc.ev1 for callee, cc in children.items() if callee != key)
    ...
    def write_call_graph(self, fp):
        for caller, callee_dict in child_calls.items():
            for callee, cc in callee_dict.items():
                ...
                name_f = node_id_for(caller)
                name_t = node_id_for(callee)
                label = f"{pct:.2f}%\\n{cc.times_called} x"
                fp.write(
                    f"\t\t\"{name_f}\" -> \"{name_t}\" "
    ...
    def write_table(self, fp):
        ...
        for rank, (neg_pct, idx, cycles) in enumerate(elements, 1):
            ...
            fp.write(f"<TD bgcolor=\"white\">{name} [T{functions[idx].tid}]</TD>\n\t\t\t\t\t")
            ...
            while True:
                ...
                if is_entry:
                    ...
                    if key not in functions:
                        ...
                        functions[key] = FunctionRecord(func_name, thread_id, address)
                    func = functions[key]
                    func.times_called += 1

                    # record child call count
                    stack = call_stacks[thread_id]
                    if stack:
                        parent = stack[-1].key
                        cc = child_calls[parent][key]
                        cc.times_called += 1

                    # push new frame
                    frame = StackFrame(thread_id, address, current_time, current_e0, current_e1)
                    frame.key = key
                    stack.append(frame)
                    ...
                    func = functions[(thread_id, frame.addr)]
                    ...
                    if stack:
                        parent = stack[-1]
                        cc = child_calls[parent.key][(thread_id, frame.addr)]
                        cc.total_cycles += delta_time
                        cc.ev0 += delta_e0
                        cc.ev1 += delta_e1

            # Final cleanup for any unmatched function entries
            for thread_id, stack in call_stacks.items():
                if not stack:
                    continue
                if args.verbose:
                    print(f"Warning: thread {thread_id} has {len(stack)} unmatched function entries detected. Processing them as incomplete frames.")
                ...
                for i, frame in enumerate(stack):
                    ...
                    func = functions[(thread_id, frame.addr)]
                    if args.verbose:
                        print(f"  Function: {func.name} [T{thread_id}] at depth {i}")
                    func.total_time += delta_time
                    func.ev0 += delta_e0
                    func.ev1 += delta_e1
